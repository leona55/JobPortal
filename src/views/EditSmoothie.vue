...
export default {
  ...
  async mounted() {
    this.routeID = this.$route.params.smoothieid;
    this.currentSmoothie = await this.$store.state.smoothiePosts.filter((post) => {
      return post.smoothieID === this.routeID;
    });
    this.$store.commit("setSmoothieState", this.currentSmoothie[0]);
  },
  methods: {
    fileChange() {
    ...
    async updateSmoothie() {
      const dataBase = await db.collection("smoothiePosts").doc(this.routeID);
